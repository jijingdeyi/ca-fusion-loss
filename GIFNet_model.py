...
class TwoBranchesFusionNet(nn.Module):
    ...
    def forward_fusion(self, x, y, trainingTag=2):
        """End-to-end fusion output in [0, 1], shape [B, 1, H, W]."""
        fea_com = self.forward_encoder(x, y)
        fea_fused = self.forward_MultiTask_branch(
            fea_com_ivif=fea_com, fea_com_mfif=fea_com, trainingTag=trainingTag
        )
        return self.forward_mixed_decoder(fea_com, fea_fused)
...
def _rgb_to_ycbcr(image):
    """Return Y, Cb, Cr with range [0, 255]."""
...
def _ycbcr_to_rgb(y, cb, cr):
    """Convert Y/Cb/Cr to RGB PIL image."""
    y_u8 = np.clip(np.round(y), 0, 255).astype(np.uint8)
    cb_u8 = np.clip(np.round(cb), 0, 255).astype(np.uint8)
    cr_u8 = np.clip(np.round(cr), 0, 255).astype(np.uint8)
    ycrcb = cv2.merge([y_u8, cr_u8, cb_u8])  # OpenCV order: Y, Cr, Cb
    rgb = cv2.cvtColor(ycrcb, cv2.COLOR_YCrCb2RGB)
    return Image.fromarray(rgb, mode="RGB")


def _to_gray_tensor(image_u8, device):
    tensor = torch.from_numpy(image_u8.astype(np.float32) / 255.0).unsqueeze(0).unsqueeze(0)
    return tensor.to(device)


def _load_model_for_inference(checkpoint_path, device):
...
def fuse_pair_to_image(
    ...
    """
    Fuse one image pair and save output.
    - If any input is RGB, output is RGB by default (Y from fused, Cb/Cr from VIS or IR).
    - Otherwise output is grayscale.
    """
    device = next(model.parameters()).device

    if ir_is_rgb:
        ir_img_rgb = Image.open(ir_path).convert("RGB")
        ir_y, ir_cb, ir_cr = _rgb_to_ycbcr(ir_img_rgb)
        ir_gray_u8 = np.clip(np.round(ir_y), 0, 255).astype(np.uint8)
    else:
        ir_gray_u8 = cv2.imread(ir_path, cv2.IMREAD_GRAYSCALE)
        ir_cb, ir_cr = None, None

    if vis_is_rgb:
        vis_img_rgb = Image.open(vis_path).convert("RGB")
        vis_y, vis_cb, vis_cr = _rgb_to_ycbcr(vis_img_rgb)
        vis_gray_u8 = np.clip(np.round(vis_y), 0, 255).astype(np.uint8)
    else:
        vis_gray_u8 = cv2.imread(vis_path, cv2.IMREAD_GRAYSCALE)
        vis_cb, vis_cr = None, None

    if ir_gray_u8 is None or vis_gray_u8 is None:
        raise ValueError("Failed to read input image(s).")

    ir_tensor = _to_gray_tensor(ir_gray_u8, device)
    vis_tensor = _to_gray_tensor(vis_gray_u8, device)

    with torch.no_grad():
        fused = model.forward_fusion(ir_tensor, vis_tensor, trainingTag=training_tag)

    fused_y = fused[0, 0].detach().cpu().numpy() * 255.0

    out_dir = os.path.dirname(output_path)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)

    if ir_is_rgb or vis_is_rgb:
        if use_vis_chroma and vis_cb is not None and vis_cr is not None:
            cb, cr = vis_cb, vis_cr
        elif ir_cb is not None and ir_cr is not None:
            cb, cr = ir_cb, ir_cr
        else:
            cb = np.full_like(fused_y, 128, dtype=np.float32)
            cr = np.full_like(fused_y, 128, dtype=np.float32)

        fused_rgb = _ycbcr_to_rgb(fused_y, cb, cr)
        fused_rgb.save(output_path)
    else:
        fused_gray = np.clip(np.round(fused_y), 0, 255).astype(np.uint8)
        cv2.imwrite(output_path, fused_gray)


def _build_cli():
    parser = argparse.ArgumentParser(description="Fuse one IR-VIS pair with GIFNet model.")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to model checkpoint.")
    parser.add_argument("--ir_path", type=str, required=True, help="Path to infrared image.")
    parser.add_argument("--vis_path", type=str, required=True, help="Path to visible image.")
    parser.add_argument("--output_path", type=str, required=True, help="Path to save fused image.")
    parser.add_argument("--ir_is_rgb", action="store_true", help="Set if IR input is RGB.")
    parser.add_argument("--vis_is_rgb", action="store_true", help="Set if VIS input is RGB.")
    parser.add_argument(
        "--training_tag",
        type=int,
        default=2,
        choices=[1, 2],
        help="1 for IVIF branch preference, 2 for MFIF branch preference.",
    )
    parser.add_argument(
        "--cpu",
        action="store_true",
        help="Force CPU inference (default auto CUDA if available).",
    )
    parser.add_argument(
        "--no_vis_chroma",
        action="store_true",
        help="Do not prioritize VIS Cb/Cr when generating RGB output.",
    )
    return parser


if __name__ == "__main__":
    cli_parser = _build_cli()
    cli_args = cli_parser.parse_args()

    infer_device = torch.device(
        "cpu" if cli_args.cpu else ("cuda" if torch.cuda.is_available() else "cpu")
    )
    model = _load_model_for_inference(cli_args.checkpoint, infer_device)
    fuse_pair_to_image(
        model=model,
        ir_path=cli_args.ir_path,
        vis_path=cli_args.vis_path,
        output_path=cli_args.output_path,
        ir_is_rgb=cli_args.ir_is_rgb,
        vis_is_rgb=cli_args.vis_is_rgb,
        training_tag=cli_args.training_tag,
        use_vis_chroma=not cli_args.no_vis_chroma,
    )
